...
// Verifies Google ID token via backend endpoint and handles user creation/login
export const googleLogin = async (idToken: string) => {
  console.log("[googleLogin] Sending ID token to backend...");
  ...
  if (!res.ok) {
    const errorMsg = await readErrorMessage(res, "Google login failed");
    console.error("[googleLogin] Error response:", res.status, res.statusText, errorMsg);
    throw new Error(errorMsg);
  }
  const auth = (await res.json()) as AuthResponse;
  console.log("[googleLogin] Success! Got tokens and user:", auth.user.email);
  return auth;
};
...
export const getDiscoverCards = async (token: string, filters: DiscoverQueryFilters = {}) => {
  ...
  const url = `${API_BASE}/discover${query ? `?${query}` : ""}`;
  const res = await fetch(url, {
    headers: { Authorization: `Bearer ${token}` },
    cache: "no-store",
  });
  if (!res.ok) {
    const errorMsg = await readErrorMessage(res, "Failed to fetch profiles");
    console.error(`[getDiscoverCards] Error fetching profiles from ${url}:`, res.status, res.statusText, errorMsg);
    throw new Error(errorMsg);
  }

...
export const users = [adminUser, ...seedUsers];
export const likes = [];
...
export const submitVerification = (userId, photoUrl) => {
...
export const setMembershipTier = (userId, tier) => {
    const user = users.find((u) => u.id === userId);
    if (!user)
        return false;
...
export const createLike = (byUserId, targetUserId, type) => {
...
export const findMutualLike = (userId, targetUserId) => likes.find((l) => l.byUserId === targetUserId && l.targetUserId === userId && (l.type === "like" || l.type === "super_like"));
export const createMatchIfNeeded = (userId, targetUserId) => {
...
export const isUserInMatch = (matchId, userId) => {
    const match = matches.find((m) => m.id === matchId);
    if (!match)
        return false;
    return match.userA === userId || match.userB === userId;
};
export const createMessage = (matchId, senderId, text) => {
    const message = {
        id: randomUUID(),
        matchId,
        senderId,
...
export const getMessagesByMatch = (matchId) => messages
...
export const getUnreadCountForMatch = (matchId, userId) => messages.filter((m) => m.matchId === matchId &&
    m.senderId !== userId &&
    !(Array.isArray(m.seenBy) ? m.seenBy : [m.senderId]).includes(userId)).length;
export const addRefreshSession = (userId, refreshToken) => {
    const session = {
        id: randomUUID(),
        userId,
        refreshToken,
...
export const hasRefreshSession = (userId, refreshToken) => refreshSessions.some((s) => s.userId === userId && s.refreshToken === refreshToken);
export const revokeRefreshSession = (refreshToken) => {
...
export const deleteUserCascade = (userId) => {
    ...
    writeSnapshot();
    return { ok: true, message: "User and related records removed" };
};
